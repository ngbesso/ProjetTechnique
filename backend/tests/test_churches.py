...
def test_member_cannot_create(client, make_user, auth_header):
    ...
    r = client.post("/churches", json={"name": "Affiliée X", "district": "Ouest"},
...
def test_mother_admin_creates_affiliate(client, make_user, auth_header, db_session):
    ...
    r = client.post("/churches", json={"name": "Église de l'Ouest", "district": "Ouest"},
...
def test_affiliate_admin_cannot_create(client, make_user, auth_header, db_session):
    ...
    aff = client.post("/churches", json={"name": "Affiliée Y", "district": "Est"},
                      headers=auth_header("boss@b.com")).json()["id"]
    chef = make_user("chef@b.com")
    admin = db_session.scalar(select(Role).where(Role.name == "admin"))
    db_session.add(UserRole(user_id=chef.id, role_id=admin.id, church_id=aff)); db_session.flush()
    r = client.post("/churches", json={"name": "Affiliée Z", "district": "Sud"},
