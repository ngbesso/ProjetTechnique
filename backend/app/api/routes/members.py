...
from app.core.email import (
    EmailSender, get_email_sender, membership_approved, membership_received,
)
...
from app.schemas.member import (
    MemberCreate, MemberList, MemberRead, MembershipRequest, MemberUpdate,
)
...
def list_members(
    ...
    rows = db.scalars(query.order_by(Member.created_at.desc()).limit(limit).offset(offset)).all()
    return MemberList(items=[MemberRead.model_validate(m) for m in rows],
...
def get_member(member_id: int, current_user: Annotated[User, Depends(get_current_user)],
               db: Annotated[Session, Depends(get_db)]):
...
def approve_member(member_id: int, background: BackgroundTasks,
                   current_user: Annotated[User, Depends(get_current_user)],
                   db: Annotated[Session, Depends(get_db)],
                   sender: Annotated[EmailSender, Depends(get_email_sender)]):
    member = _load(db, member_id)
...
def reject_member(member_id: int, current_user: Annotated[User, Depends(get_current_user)],
                  db: Annotated[Session, Depends(get_db)]):
    member = _load(db, member_id)
    _ensure(current_user, member, "member:approve")
...
def deactivate_member(member_id: int, current_user: Annotated[User, Depends(get_current_user)],
                      db: Annotated[Session, Depends(get_db)]):
    member = _load(db, member_id)
    _ensure(current_user, member, "member:update")
